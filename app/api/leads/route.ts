...
export async function POST(request: Request) {
  ...
  if (!profile || profile.role !== 'GUEST') {
    return NextResponse.json(fail('UNAUTHORIZED', 'Guest login required'), {
      status: 401,
    });
  }
  ...
  if (!assetId) {
    return NextResponse.json(fail('INVALID', 'assetId required'), { status: 400 });
  }

  const rl = await rateLimit(`lead:${profile.id}:${assetId}`, 3, 60 * 60);
  if (!rl.success) {
    return NextResponse.json(fail('RATE_LIMIT', 'Too many lead requests'), {
      status: 429,
    });
  }
  ...
  if (!asset) {
    return NextResponse.json(fail('NOT_FOUND', 'Asset not available'), {
      status: 404,
    });
  }
