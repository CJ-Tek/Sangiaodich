...
export default async function SaleHomePage({
  ...
  const t = await getTranslations('sale.home');
  ...
  const periodLabel = t('monthLabel', {
    month: period.month,
    year: period.year,
  });
  const firstName =
  ...
  return (
    <Stack gap={40}>
      ...
      <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
        <StatCard
          label={t('actualRevenue')}
          value={formatVnd(actualRevenue, locale)}
          hint={periodLabel}
          emphasis="hero"
        />
        <StatCard
          label={t('expectedRevenue')}
          value={formatVnd(expectedRevenue, locale)}
          hint={t('pipelineHint')}
        />
        <StatCard
          label={t('successfulBookings')}
          value={periodSuccessful}
          hint={t('lifetimeHint', {
            period: periodLabel,
            count: lifetimeSuccessful || 0,
          })}
        />
      </SimpleGrid>
        ...
        <Group justify="space-between">
          ...
          {unreadLeads > 0 ? (
            <LinkButton href="/sale/leads" variant="light" color="vbnbGreen" size="xs">
              {unreadLeads > UNREAD_LEAD_CAP
                ? t('unreadLeadsCapped', { count: UNREAD_LEAD_CAP })
                : t('unreadLeads', { count: unreadLeads })}
            </LinkButton>
          ) : null}
        </Group>
        {!upcoming?.length ? (
