...
export default async function EditOwnerAssetPage({
  ...
  const t = await getTranslations('owner.assetEdit');
  ...
  return (
    <Stack gap="lg">
      <PageHeader
        title={t('title', { title: asset.title })}
      ...
      <SurfaceCard maw={720}>
        <AssetForm
          ...
          initial={{
            ...
            discountRules: discountTiers.map((tier) => ({
              minCheckedOutCount: tier.minCheckedOutCount,
              costDiscountPercent: tier.costDiscountPercent,
            })),
          }}
        />
      </SurfaceCard>
